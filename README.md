...
- `extensions/subagents` — non-blocking background Pi subagents (`list_agents`, `run_agent`, `stop_agent` tools) with completion callbacks, persisted transcripts, repo-root `.pi/worktree.json` copy/setup support, normalized postCopy confirmation/env hardening, and guarded symlink copies
