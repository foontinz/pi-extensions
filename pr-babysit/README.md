...
After every successful idle poll, and again before every agent run, a clean managed worktree is fast-forwarded to the latest pushed head commit and the PR base branch (`origin/<baseRefName>`, e.g. `main`) is automatically merged into the head branch. When the merge introduces new commits they are pushed straight to the PR head ref, so the pull request stays continuously up to date with its base even when no comments or CI events arrive. Idle synchronization and event dispatch are mutually exclusive, awaited paths in the poll loop; a per-PR cross-process worktree lock also prevents periodic synchronization, agent execution, and worktree removal from overlapping, while a repository lock serializes shared remote-ref updates across different PR worktrees. The merge commit message is git's default unless `baseMergeMessage` is configured (needed when the repository enforces commit-message rules). A merge that stays local because a push failed is retried on the next sync. Fast-forwards, real merges, and "already merged" states are all handled; a merge that conflicts is aborted and left for the dispatched agent to resolve rather than blocking the run. If the worktree has uncommitted local changes, synchronization (and the base merge) is skipped until it is clean. Agent-run sync actions are recorded in the run prompt and `runs/<run-id>/meta.json`; noteworthy idle sync actions are logged in the watcher pane.
...
Agent subprocesses use pi's non-streaming text print mode. Each run keeps only the final assistant response in `runs/<run-id>/response.txt`; response artifacts are capped at 2 MiB and retain their beginning and end with an explicit marker when that cap is reached. The complete durable pi session remains available under that PR's `sessions/` directory. Token-level JSON event streams are intentionally not retained because their repeated partial-message snapshots can grow quadratically.
